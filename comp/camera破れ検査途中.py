...
class Camera:
    ...
    def manage_image_processing(self):
        ...
        while(1):
            ...
            if self.is_start_kensa:
                self.judge_kensa(self.frame_oshidashi)
                self.is_start_kensa = False
    ...
    ##検査部の袋の破れ有無判定
    def judge_kensa(self, image):
        print("検査部の袋の破れ有無判定")
        self.measurement_start = time.time()
        
        cv2.imwrite(self.output_kensa_image_filepath_upleft, image)
        ##画像の読み込み
        self.image = cv2.imread(self.output_kensa_image_filepath_upleft,0)
        ...
        image_color = cv2.imread(self.output_kensa_image_filepath_downleft,1)
        ...
        #cv2.waitKey(0)
        cv2.imwrite(self.output_kensa_image_filepath_downleft, image_color)
