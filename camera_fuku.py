...
class Camera:
    ...
    def __init__(self):
        ...
        self.output_kensa1 = "C:/startfile/g52024/image/output_kensa1.jpg"
        self.output_kensa2 = "C:/startfile/g52024/image/output_kensa2.jpg"
        self.output_kensa3 = "C:/startfile/g52024/image/output_kensa3.jpg"
        self.output_kensa4 = "C:/startfile/g52024/image/output_kensa4.jpg"
        ...
        self.cap_fukuro = cv2.VideoCapture(6)#袋セット部カメラ
        self.cap_qr = cv2.VideoCapture(5)#qr部カメラ
        self.cap_kensa1 = cv2.VideoCapture(0)#検査部カメラ左上
        self.cap_kensa2 = cv2.VideoCapture(1)#検査部カメラ右上
        self.cap_kensa3 = cv2.VideoCapture(2)#検査部カメラ左下
        self.cap_kensa4 = cv2.VideoCapture(4)#検査部カメラ右下
        ...
        self.cap_oshidashi.set(cv2.CAP_PROP_EXPOSURE, -5)
    ...
    def manage_image_processing(self):
        ...
        while(1):
            ...
            if self.is_start_kensa:
                ...
                self.judge_kensa(self.frame_kensa1, self.output_kensa1)#検査部の袋穴あき検査モジュールの実行
                self.judge_kensa(self.frame_kensa2, self.output_kensa2)#検査部の袋穴あき検査モジュールの実行
                self.judge_kensa(self.frame_kensa3, self.output_kensa3)#検査部の袋穴あき検査モジュールの実行
                self.judge_kensa(self.frame_kensa4, self.output_kensa4)#検査部の袋穴あき検査モジュールの実行
    ...
    def judge_fukuro(self, image):
        ...
        image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)#画像をグレースケールに変換
        image = cv2.bitwise_and(image, mask)#マスク処理
        thresh_value, image_area = cv2.threshold(image, 150, 255, cv2.THRESH_BINARY)#2値化処理
        whitePixels = cv2.countNonZero(image_area)#白色の部分を算出
        blackPixels = image_size - whitePixels#黒色の部分を算出
